# Programmatic layout gate: re-executes batching_effects.py in-process, then checks every
# Text artist in every axes for (a) pairwise text-on-text overlaps and (b) text extending
# beyond its axes bbox. Prints any violations with locations.
...
g = runpy.run_path(r"C:\Users\barry\Desktop\ml-infra-journey\physics-llm-inference-working"
                   r"\batching-effects\batching_effects.py")
fig = g["fig"]
fig.canvas.draw()
...
panel_names = ["A intensity", "B roofline", "C scaling",
               "D tradeoff", "E ceiling", "F takeaway",
               "G continuous", "H knobs", "I sources"]
issues = 0
global_texts = []
