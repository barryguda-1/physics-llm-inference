# Programmatic layout gate: re-executes gpu_architecture.py in-process, then
# checks every Text artist in every axes for (a) pairwise text-on-text overlaps
# and (b) text extending beyond its axes bbox. Prints any violations with
# locations. All extents are measured through one renderer after a single
# draw() so axes and text boxes share the same dpi; bboxes are frozen to avoid
# lazy re-eval.
...
g = runpy.run_path(r"C:\Users\barry\Desktop\ml-infra-journey\physics-llm-inference-working\gpu-architecture\gpu_architecture.py")
fig = g["fig"]
fig.canvas.draw()
...
panel_names = ["A chip", "B grid-warps", "C pyramid", "D reuse", "E occupancy", "F takeaway"]
issues = 0
global_texts = []

# titles live outside ax.texts and span panel gaps -> collect globally
title_items = []
for idx, ax in enumerate(fig.axes):
    if ax.title.get_text().strip():
        bb = frozen(mtext.Text.get_window_extent(ax.title, renderer))
        title_items.append((f"[{panel_names[idx]} title]", ax.title.get_text()
                            .replace("\n", " / ")[:46], bb))
...
for idx, ax in enumerate(fig.axes):
    ...
    for label, bb in texts:
        global_texts.append((name, label, bb))
...
# (c) titles vs titles, and titles vs every panel's texts/ticks (cross-panel)
all_items = title_items + [(p, l, b) for p, l, b in global_texts]
for i in range(len(title_items)):
    for j in range(len(all_items)):
        if title_items[i] is all_items[j]:
            continue
        x0 = max(title_items[i][2].x0, all_items[j][2].x0)
        x1 = min(title_items[i][2].x1, all_items[j][2].x1)
        y0 = max(title_items[i][2].y0, all_items[j][2].y0)
        y1 = min(title_items[i][2].y1, all_items[j][2].y1)
        if x1 - x0 > 3 and y1 - y0 > 3:
            issues += 1
            print(f"OVERLAP  {title_items[i][0]} '{title_items[i][1]}'  <->  "
                  f"[{all_items[j][0]}] '{all_items[j][1]}'  ({x1-x0:.0f}x{y1-y0:.0f} px)")
